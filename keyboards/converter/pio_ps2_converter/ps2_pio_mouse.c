...
void    ps2m_host_init(void);
uint8_t ps2m_host_send(uint8_t data);
uint8_t ps2m_host_recv_response(void);
uint8_t ps2m_host_recv(void);

#if !defined(MCU_RP)
#    error PIO Driver is only available for Raspberry Pi 2040 MCUs!
#endif

#if defined(PS2M_ENABLE)
#    if defined(PS2M_MOUSE_ENABLE)
#        if !defined(PS2M_MOUSE_USE_REMOTE_MODE)
...
#define PS2M_WRAP_TARGET 0
#define PS2M_WRAP 20

// clang-format off
static const uint16_t ps2m_program_instructions[] = {
            //     .wrap_target
    0x00c7, //  0: jmp    pin, 7
    0xe02a, //  1: set    x, 10
...
static const struct pio_program ps2m_program = {
    .instructions = ps2m_program_instructions,
    .length       = 21,
    .origin       = -1,
};
...
uint8_t ps2m_error = PS2_ERR_NONE;
...
void ps2m_host_init(void) {
    ...
    if (state_machine < 0) {
        ...
        ps2m_error = PS2_ERR_NODATA;
        return;
    }

    uint offset = pio_add_program(pio, &ps2m_program);

    pio_sm_config c = pio_get_default_sm_config();
    sm_config_set_wrap(&c, offset + PS2M_WRAP_TARGET, offset + PS2M_WRAP);

    // Set pindirs to input (output enable is inverted below)
    pio_sm_set_consecutive_pindirs(pio, state_machine, PS2M_DATA_PIN, 2, true);
    sm_config_set_clkdiv(&c, (float)clock_get_hz(clk_sys) / (200.0f * KHZ));
    sm_config_set_set_pins(&c, PS2M_DATA_PIN, 2);
    sm_config_set_out_pins(&c, PS2M_DATA_PIN, 1);
    sm_config_set_out_shift(&c, true, true, 10);
    sm_config_set_in_shift(&c, true, true, 11);
    sm_config_set_jmp_pin(&c, PS2M_CLOCK_PIN);
    sm_config_set_in_pins(&c, PS2M_DATA_PIN);
    ...
    palSetLineMode(PS2M_DATA_PIN, pin_mode);
    palSetLineMode(PS2M_CLOCK_PIN, pin_mode);
...
uint8_t ps2m_host_send(uint8_t data) {
    ...
    return ps2m_host_recv_response();
}

static uint8_t ps2m_get_data_from_frame(uint32_t frame) {
    ...
    if (start_bit != 0) {
        ps2m_error = PS2_ERR_STARTBIT1;
        return 0;
    }

    if (parity_bit != bit_parity(data)) {
        ps2m_error = PS2_ERR_PARITY;
        return 0;
    }

    if (stop_bit != 1) {
        ps2m_error = PS2_ERR_STARTBIT2;
        return 0;
    }
...
uint8_t ps2m_host_recv_response(void) {
    ...
    return ps2m_get_data_from_frame(frame);
}

#ifdef BUFFERED_MODE_ENABLE

bool pbufm_has_data(void) {
    osalSysLock();
...
uint8_t ps2m_host_recv(void) {
    ...
    uint8_t has_data = pbufm_has_data();
    if (has_data) {
        msg = ibqReadTimeout(&pio_rx_queue, (uint8_t*)&frame, sizeof(uint32_t), TIME_MS2I(100));
        if (msg < MSG_OK) {
            ps2m_error = PS2_ERR_NODATA;
    ...
    return frame != 0 ? ps2m_get_data_from_frame(frame) : 0;
}
